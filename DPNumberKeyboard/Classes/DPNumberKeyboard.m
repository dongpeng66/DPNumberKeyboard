...
typedef NS_ENUM(NSUInteger, DPNumberKeyboardButton) {
    DPNumberKeyboardButtonNumberMin,
    DPNumberKeyboardButtonNumberMax = DPNumberKeyboardButtonNumberMin + 10, // Ten digits.
    DPNumberKeyboardButtonBackspace,
    DPNumberKeyboardButtonDone,
    DPNumberKeyboardButtonSpecial,
    DPNumberKeyboardButtonDecimalPoint,
    DPNumberKeyboardButtonNone = NSNotFound,
};

@interface DPNumberKeyboard () <UIInputViewAudioFeedback>
...
@interface _DPNumberKeyboardButton : UIButton

+ (_DPNumberKeyboardButton *)keyboardButtonWithStyle:(DPNumberKeyboardButtonStyle)style;

// The style of the keyboard button.
@property (assign, nonatomic) DPNumberKeyboardButtonStyle style;
...
+ (id)DP_currentFirstResponder
{
    currentFirstResponder = nil;
    [[UIApplication sharedApplication] sendAction:@selector(DP_findFirstResponder:) to:nil from:nil forEvent:nil];
    return currentFirstResponder;
}
#pragma clang diagnostic pop

- (void)DP_findFirstResponder:(id)sender
{
...
@implementation DPNumberKeyboard

static const NSInteger DPNumberKeyboardRows = 4;
static const CGFloat DPNumberKeyboardRowHeight = 55.0f;
static const CGFloat DPNumberKeyboardPadBorder = 7.0f;
static const CGFloat DPNumberKeyboardPadSpacing = 8.0f;
...
- (void)_commonInit
{
    ...
    const NSInteger numberMin = DPNumberKeyboardButtonNumberMin;
    const NSInteger numberMax = DPNumberKeyboardButtonNumberMax;
    ...
    for (DPNumberKeyboardButton key = numberMin; key < numberMax; key++) {
        UIButton *button = [_DPNumberKeyboardButton keyboardButtonWithStyle:DPNumberKeyboardButtonStyleWhite];
    ...
    UIImage *backspaceImage = [self.class _keyboardImageNamed:@"DPNumberKeyboardDeleteKey.png"];
    UIImage *dismissImage = [self.class _keyboardImageNamed:@"DPNumberKeyboardDismissKey.png"];
    
    UIButton *backspaceButton = [_DPNumberKeyboardButton keyboardButtonWithStyle:DPNumberKeyboardButtonStyleGray];
    ...
    [(_DPNumberKeyboardButton *)backspaceButton addTarget:self action:@selector(_backspaceRepeat:) forContinuousPressWithTimeInterval:0.15f];
    
    [buttonDictionary setObject:backspaceButton forKey:@(DPNumberKeyboardButtonBackspace)];
    
    UIButton *specialButton = [_DPNumberKeyboardButton keyboardButtonWithStyle:DPNumberKeyboardButtonStyleGray];
    
    [buttonDictionary setObject:specialButton forKey:@(DPNumberKeyboardButtonSpecial)];
    
    UIButton *doneButton = [_DPNumberKeyboardButton keyboardButtonWithStyle:DPNumberKeyboardButtonStyleDone];
    [doneButton.titleLabel setFont:doneButtonFont];
    [doneButton setTitle:@"确定" forState:UIControlStateNormal];
    
    [buttonDictionary setObject:doneButton forKey:@(DPNumberKeyboardButtonDone)];
    
    UIButton *decimalPointButton = [_DPNumberKeyboardButton keyboardButtonWithStyle:DPNumberKeyboardButtonStyleWhite];
    ...
    [buttonDictionary setObject:decimalPointButton forKey:@(DPNumberKeyboardButtonDecimalPoint)];
    ...
    [self setReturnKeyButtonStyle:DPNumberKeyboardButtonStyleDone];
...
{
    __block DPNumberKeyboardButton keyboardButton = DPNumberKeyboardButtonNone;
    
    [self.buttonDictionary enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        DPNumberKeyboardButton k = [key unsignedIntegerValue];
        if (button == obj) {
            keyboardButton = k;
            *stop = YES;
        }
    ...
    if (keyboardButton == DPNumberKeyboardButtonNone) {
        return;
    }
    
    // Get first responder.
    id <UIKeyInput> keyInput = self.keyInput;
    id <DPNumberKeyboardDelegate> delegate = self.delegate;
    
    if (!keyInput) {
        return;
    }
    //    NSLog(@"%@",keyInput);
    UITextField *textFileld=(UITextField *)keyInput;
    ...
    if (keyboardButton >= numberMin && keyboardButton < numberMax) {
        ...
        for (int i=1; i<=textFileld.text .length; i++) {
            [arr addObject:[textFileld.text  substringWithRange:NSMakeRange(i-1, 1)]];
        ...
        if(myIndex!=0){
            ...
            if (textFileld.text .length-myIndex<2) {
    ...
    // Handle backspace.
    else if (keyboardButton == DPNumberKeyboardButtonBackspace) {
    ...
    // Handle done.
    else if (keyboardButton == DPNumberKeyboardButtonDone) {
    ...
    // Handle special key.
    else if (keyboardButton == DPNumberKeyboardButtonSpecial) {
    ...
    // Handle .
    else if (keyboardButton == DPNumberKeyboardButtonDecimalPoint) {
        ...
        if( textFileld.text.length > 0 && ![textFileld.text containsString:@"."]){
    ...
    if ([self.delegate respondsToSelector:@selector(numberKeyboardTextString:withTextString:)]) {
        [self.delegate numberKeyboardTextString:self withTextString:textFileld.text];
    }
}
...
- (id<UIKeyInput>)keyInput
{
    ...
    keyInput = [UIResponder DP_currentFirstResponder];
...
- (void)configureSpecialKeyWithImage:(UIImage *)image actionHandler:(dispatch_block_t)handler
{
    ...
    UIButton *button = self.buttonDictionary[@(DPNumberKeyboardButtonSpecial)];
...
- (void)setReturnKeyTitle:(NSString *)title
{
    if (![title isEqualToString:self.returnKeyTitle]) {
        UIButton *button = self.buttonDictionary[@(DPNumberKeyboardButtonDone)];
        if (button) {
...
- (void)setReturnKeyButtonStyle:(DPNumberKeyboardButtonStyle)style
{
    if (style != _returnKeyButtonStyle) {
        _returnKeyButtonStyle = style;
        
        _DPNumberKeyboardButton *button = self.buttonDictionary[@(DPNumberKeyboardButtonDone)];
        if (button) {
            button.style = style;
        }
    }
...
NS_INLINE CGRect DPButtonRectMake(CGRect rect, CGRect contentRect, UIUserInterfaceIdiom interfaceIdiom){
    rect = CGRectOffset(rect, contentRect.origin.x, contentRect.origin.y);
    
    if (interfaceIdiom == UIUserInterfaceIdiomPad) {
        CGFloat inset = DPNumberKeyboardPadSpacing / 2.0f;
...
#define DPRound round
#else
#define DPRound roundf
#endif
...
- (void)layoutSubviews
{
    ...
    const CGFloat spacing = (interfaceIdiom == UIUserInterfaceIdiomPad) ? DPNumberKeyboardPadBorder : 0.0f;
    ...
    const CGRect contentRect = (CGRect){
        .origin.x = DPRound((CGRectGetWidth(bounds) - width) / 2.0f),
        .origin.y = spacing,
        .size.width = width,
    ...
    const CGFloat rowHeight = DPNumberKeyboardRowHeight;
    ...
    for (DPNumberKeyboardButton key = numberMin; key < numberMax; key++) {
        ...
        [button setFrame:DPButtonRectMake(rect, contentRect, interfaceIdiom)];
    }
    
    // Layout special key.
    UIButton *specialKey = buttonDictionary[@(DPNumberKeyboardButtonSpecial)];
    if (specialKey) {
        CGRect rect = (CGRect){ .size = numberSize };
        rect.origin.y = numberSize.height * 3;
        
        [specialKey setFrame:DPButtonRectMake(rect, contentRect, interfaceIdiom)];
    }
    
    // Layout decimal point.
    UIButton *decimalPointKey = buttonDictionary[@(DPNumberKeyboardButtonDecimalPoint)];
    if (decimalPointKey) {
        CGRect rect = (CGRect){ .size = numberSize };
        rect.origin.y = numberSize.height * 3;
        rect.origin.x = numberSize.width * 2;
        
        [decimalPointKey setFrame:DPButtonRectMake(rect, contentRect, interfaceIdiom)];
        
        decimalPointKey.hidden = !allowsDecimalPoint;
    }
    
    // Layout utility column.
    const int utilityButtonKeys[2] = { DPNumberKeyboardButtonBackspace, DPNumberKeyboardButtonDone };
    ...
    for (NSInteger idx = 0; idx < sizeof(utilityButtonKeys) / sizeof(int); idx++) {
        DPNumberKeyboardButton key = utilityButtonKeys[idx];
    ...
    if (interfaceIdiom != UIUserInterfaceIdiomPad) {
        ...
        [separatorViews enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            ...
            [separator setFrame:DPButtonRectMake(rect, contentRect, interfaceIdiom)];
        }];
    }
}
...
- (CGSize)sizeThatFits:(CGSize)size
{
    ...
    size.height = DPNumberKeyboardRowHeight * DPNumberKeyboardRows + (spacing * 2.0f);
...
@interface _DPNumberKeyboardButton ()
...
@implementation _DPNumberKeyboardButton

+ (_DPNumberKeyboardButton *)keyboardButtonWithStyle:(DPNumberKeyboardButtonStyle)style
{
    _DPNumberKeyboardButton *button = [self buttonWithType:UIButtonTypeCustom];
    button.style = style;
...
- (void)setStyle:(DPNumberKeyboardButtonStyle)style
{
    if (style != _style) {
        _style = style;
...
{
    const UIUserInterfaceIdiom interfaceIdiom = UI_USER_INTERFACE_IDIOM();
    const DPNumberKeyboardButtonStyle style = self.style;
    
    UIColor *fillColor = nil;
    UIColor *highlightedFillColor = nil;
    if (style == DPNumberKeyboardButtonStyleWhite) {
        fillColor = [UIColor whiteColor];
        highlightedFillColor = [UIColor colorWithRed:0.82f green:0.837f blue:0.863f alpha:1];
    } else if (style == DPNumberKeyboardButtonStyleGray) {
        if (interfaceIdiom == UIUserInterfaceIdiomPad) {
    ...
    } else if (style == DPNumberKeyboardButtonStyleDone) {
        fillColor =  [UIColor colorWithRed:246/255.0 green:76/255.0 blue:62/255.0 alpha:1/1.0];
        highlightedFillColor = [UIColor whiteColor];
    }
    
    UIColor *controlColor = nil;
    UIColor *highlightedControlColor = nil;
    if (style == DPNumberKeyboardButtonStyleDone) {
